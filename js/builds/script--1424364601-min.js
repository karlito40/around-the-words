...
DataManager.prototype.getApi = function()
{
	var apiName = (this.online) ? "OnlineApi" : "OfflineApi";
...
LoadingScene.prototype.loadPlayer = function(){
	...
	if(EXPORT_PLATFORM == 'facebook') {
		console.log('dt', dt);
		ATW.App.initPlayer(dt);
		ATW.App.getPlayer().setLanguage(ATW.config.myLang, ATW.config.myLocale);
		...
		ATW.App.getDataManager().getApi().call('User', 'GET', {
			...
			ATW.App.initPlayer(res.me);
			ATW.App.getPlayer().setLanguage(ATW.config.myLang, ATW.config.myLocale);

			self.loadedDatas = true;
			self.handleCompleteLoad();
...
LoadingScene.prototype.globeAnim = function(cb, reverse, timeScale){
	...
	var toAlpha = (!reverse) ? 1 : 0;

	var tl = new TimelineMax();
	tl.to(map, 3, {alpha: toAlpha}, 'start');
	tl.to(logo, 2, {alpha:toAlpha}, 'start+=0.7');
...
LoadingScene.prototype.nextScene = function(){
	var self = this;
	console.log('yoooo do nextScene');
...
views.world_scene = function build_world_scene(builder) {
	...
	if(ATW.isMobile()) {
		...
		builder.createWorld = function(i) {
			...
			for(var worldID in worldDatas){
				if(c == i) {
					var cWorld = worldDatas[worldID]
						, world = new UI.World(cWorld, true)
...
{
	if(typeof LANG_TO_LOCAL[lang] == "undefined") return false;

	this.lang = lang;
	this.locale = LANG_TO_LOCAL[lang];
...
User.prototype.loadLang = function()
{
	Gettext.setLocale(this.locale);
...
if(!validLanguage[myLang]) {
	myLang = 'en';

};

var myLocale = validLanguage[myLang];
Gettext.setLocale(myLocale);
loadTrad();

namespace.config.myLang = myLang;
namespace.config.myLocale = myLocale;
